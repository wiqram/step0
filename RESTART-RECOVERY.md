...
| Apps report broker creds/platform keys "not configured" though Mongo has them; `kv/yolo/followers/` empty | **Vault KV data missing** — the Vault PV is a dynamic hostPath in the VM's `/tmp` (reclaim Delete); a minikube rebuild wipes it and vaultSync re-seeds only declarative paths | `kubectl -n vault exec vault-0 -- vault kv list kv/yolo/followers` → "No value found" | restore newest snapshot: `kubectl -n vault scale sts vault --replicas=0`, then on the node untar `minikube-mnt/vault-backups/vault-data-<latest>.tgz` into the PV hostPath (`kubectl -n vault get pv $(kubectl -n vault get pvc data-vault-0 -o jsonpath='{.spec.volumeName}') -o jsonpath='{.spec.hostPath.path}'` — via `minikube ssh`), scale back to 1, unseal (auto-unsealer). Runtime keys saved after the snapshot are gone — followers re-connect brokers; admin re-enters platform keys in /admin/settings |
