...
| **Dev box** `kubectl`/IntelliJ Services hangs then times out; prod itself is completely healthy | **Docker's direct-routing DROP is back without our ACCEPT.** Docker ≥28 (here 29.7.2) blocks off-host traffic to container IPs via `raw`/PREROUTING at priority **−300** — before conntrack, long before `FORWARD` — so the `DOCKER-USER` rules are bypassed entirely. Docker rebuilds that table on **every dockerd start**, so a docker restart/upgrade wipes our ACCEPT. Before 2026-08-07 the unit was `After=docker.service` only, so it re-applied at *boot* but **not** on a docker restart; it now has `PartOf=docker.service`. | `sudo iptables -t raw -L PREROUTING -n -v` → the `DROP … 172.16.238.2` counter is **climbing** and there is no ACCEPT above it. Or just `sudo ./verify-recovery.sh` (§3 asserts this rule). | `sudo ./enable-devbox-kube-access.sh` (idempotent). If it keeps vanishing, the unit is stale — `sudo ./enable-devbox-kube-access.sh --install` rewrites it *and* refreshes the `/usr/local/sbin` copy the unit actually runs. Durable fix = recreate `5million` with `trusted_host_interfaces` (**plan.md §6a**). |
| **Dev box** `kubectl` fails fast with `x509: certificate signed by unknown authority` or a bare `Unauthorized` (note: **fails**, doesn't hang — that distinguishes it from the row above) | **Stale CA — but NOT from a plain rebuild.** The CA lives in `~/.minikube/ca.crt`, outside the cluster, and minikube reuses it: a cold `start-scratch.sh` leaves it untouched (verified 2026-08-07 — the dev box kept working straight through one). It changes only when **`~/.minikube` is recreated**: fresh OS install, `minikube delete --purge`, or wiping the dir. The dev box's kubeconfig embeds the *old* cert and keeps looking valid. Reads like a firewall fault and sends you auditing rules that are fine. | Compare fingerprints: `grep -m1 certificate-authority-data ~/.kube/config \| sed 's/.*: //' \| base64 -d \| openssl x509 -noout -fingerprint` on dev vs `openssl x509 -in ~/.minikube/ca.crt -noout -fingerprint` on prod | On prod `./enable-devbox-kube-access.sh --emit-kubeconfig`, copy over, then on dev `./devbox-connect-prod.sh kubeconfig <file>`. **Use a current copy of that script** — the pre-2026-08-07 version merged in the wrong order and silently kept the dead CA while reporting success. Then refresh IntelliJ's Services window (it caches). |
