...
| `alerting-pipeline-watch.sh` | `*/5` cloud cron: Prometheus up, Alertmanager up, **Loki `/ready` (added 2026-09-05 — the yolo Grafana rules are all Loki-backed and `KeepLast`, so a dead Loki makes them silent, not red; this is the one probe that says so)**, `Watchdog` alert firing, notification failures → ntfy when sustained. Runs **outside** the cluster; renamed from `resource-crunch-watch.sh` 2026-08-04 when the resource thresholds moved into Alertmanager. `--status` prints everything without sending. See §7a/§7b. |
| `docker-runtime-upgrade.sh` | **Root systemd timer, Saturday 04:32 Europe/London** (added 2026-09-05, OBS-ALERT-EXECERR): `apt-mark hold` on the six docker-ce/containerd packages the rest of the week, then unhold → `apt-get install --only-upgrade` → re-hold in the market-closed window, wait for node Ready + every `yolo` Deployment at replica count + Loki `/ready`, ONE ntfy on `yolo-private-cloud-platform` only when something was upgraded. Why: a docker postinst restarts containerd+docker and with them every pod on this single-node cluster; Docker's repo is not in unattended-upgrades' origins, so before this the only upgrade path was a human's `apt upgrade` at a random time. `--install` (also run by `restore-scratch.sh` phase 8), `--status`, `--check`, `--force`. |
