...
| other `reduce-*.sh`, `delete-docker-reg-images.sh`, `remove-old-snaps.sh` | Disk/space maintenance. `reduce-docker-minikube-space.sh` is the **emergency hammer** (`docker system prune -a`, host + node) — never schedule it; it forces a re-pull/rebuild of everything. |
