...
| `tailscale-access.sh` | **Off-LAN access to the administrative vhosts (SEC-EDGE-ALLOWLIST).** nginx restricts jenkins/grafana/vault/registry/prometheus/NPM-admin to an allowlisted *source* set (nginx repo `data/nginx/custom/http_top.conf`), so a phone on cellular gets **403**. That range can't be allowlisted — carriers use CGNAT, so it's shared by thousands of strangers and rotates. Instead `tailscaled` advertises **this host's own public IP as a `/32` subnet route**: an enrolled device keeps resolving the vhosts to the public IP exactly as before but routes *that one address* over the tailnet. URLs unchanged, no new inbound port, and access becomes revocable per device. ⚠️ **Three things that look like style and are not.** (1) **`--accept-dns=false`, asserted on every run** — every image pull targets `container-registry.traderyolo.com`, which resolves to the public IP and hairpins back through the router; letting Tailscale own this host's DNS risks **cluster-wide `ImagePullBackOff`** traced to a VPN client. (2) **Advertising a route is not approving it** — until it's ticked in the console (or auto-approved by ACL) `PrimaryRoutes` is `null` and enrolled devices silently don't use it, with no error at either end. (3) **`tailscaled.state` IS the machine's identity** — `backup-minikube-mnt.sh` captures it and `restore-scratch.sh` phase 4h stages it to `/mnt/minikube-backups/tailscale-state-restore/`, so a rebuilt box rejoins as the **same** machine (same `100.x`, same already-approved route) with **no browser login**. Without it DR needs `TAILSCALE_AUTHKEY` in `.env` (keys expire, ≤90d) and then a human. ⚠️ **SNAT makes the per-device `/32`s in the nginx geo block inert** — subnet-route traffic is rewritten to this host's address and hairpins, so nginx logs a phone as `213.48.246.115`; a new tailnet device needs **no** nginx change, and **revocation is in the Tailscale console, not that file**. Called by `restore-scratch.sh` phase 8 (`--install`) and `start-scratch.sh` (`--ensure`, drift-check only). Never load-bearing: every caller ignores a non-zero exit. `--install` / `--ensure` / `--status` / `--uninstall` / `--dry-run`. See nginx repo `docs/edge-exposure.md` §4. |
...
| `devbox-metrics.sh` | **Dev-box side: exposes the dev box's metrics so prod Prometheus can scrape it over 10GbE** (run on `vik@10.10.10.2`; `--install` / `--status` / `--uninstall`). Feeds the `Dev box — overview` Grafana dashboard (CPU, memory, disk, network, CPU/NVMe temps, GPU). ⚠️ **Every design choice here follows from "`vik` has sudo but NOT passwordless sudo"**: node-exporter runs as a **container** (`vik` is in the `docker` group) rather than the apt package + systemd unit, with `--restart unless-stopped` for boot persistence; GPU metrics come from an **`nvidia-smi` textfile collector** on a **user** crontab, because a DCGM/GPU exporter container needs `nvidia-container-toolkit`, which is **not installed** here (`docker info` lists only `runc`, so `docker run --gpus all` fails) and installing it needs root. Binds to **`10.10.10.2:9100`, not `0.0.0.0`** — node-exporter has no auth and the box is also on the house LAN (`192.168.50.161`); same narrow-the-reachability reasoning as SEC-LOKI-NODEPORT. ⚠️ **Nothing on the platform may depend on this box** — it dual-boots Windows and is legitimately off much of the time. Prod side is `kube-prometheus manifests/devbox-scrapeConfig.yaml` (a static `ScrapeConfig`; an unreachable static target is not a config error, so Prometheus starts and reloads fine with the box absent) plus a `TargetDown{job="devbox"}` → `null` Alertmanager route so it never pages. Verified by stopping the exporter: `up` → 0, the dashboard's Online/**OFFLINE** panel flips, prod scraping/Grafana/Prometheus all unaffected, and it recovers by itself. |
...
| `verify-recovery.sh` | **Read-only post-restore survey** (mutates nothing, with ONE documented exception: the Vault ACL sweep mints a 90s throwaway token per policy and revokes it, because `vault token capabilities` needs a token — no secret is read or written and no real path is test-written; `SKIP_VAULT_ACL=1` opts out). Confirms the facts a fresh box can silently get wrong: fixed cluster IPs (minikube node `172.16.238.2`, NPM `.10`, 5million subnet `172.16.0.0/16`), backup-NAS reachability + exports (WD Cloud DR `.169` NFS, WD My Cloud `.68`/`.251` SMB), the dev↔prod 10GbE `/30` link + watchdog, and host/DNS/service/cron state (hostname, public IP vs `jenkins.traderyolo.com` A-record, Vault unsealed, both backup crons + cloud crontab). Also guards **SEC-EDGE-ALLOWLIST**: FAILs if the nginx `custom/` files are missing or lost the public-IP trust entry, and — the check that actually matters, since NPM regenerates its confs from its DB — probes the **live deny branch from a container on the docker bridge** (a deliberately untrusted vantage; a `curl` from this host proves nothing) and FAILs on anything but 403. Tailscale is WARN-only except `accept-dns` enabled, which is a **FAIL** because it can take the whole cluster down via the registry hairpin. Also guards **BUG-VAULT-PLATFORM-KEY-403** (§ Vault ACL): mints a throwaway 90s token per policy and asserts every path an app WRITES at runtime is actually writable — read-granted-write-denied has NO symptom (app up, reads fine, health checks green, while an admin key upload 403s into a store that stays empty), so neither "Vault is unsealed" nor "all pods Running" can see it. Plus the inverse half: app runtimes stay read-only, each Jenkins AppRole can sync its own tenant and no other, and no app token can reach its own SOPS age key. `VR_YOLO_POLICY` exists ONLY so the write-path branch can be proven to still FAIL. Prints PASS/WARN/FAIL + a "detected values" digest of the env-specific facts; exit 1 on any FAIL. Expected values are env-overridable constants at the top. Called best-effort by `restore-scratch.sh` phase 9; run by hand anytime after a reboot/crash. |
| `ntfy-lib.sh` | **The push-notification registry + publisher.** Sourced (never executed) by every job that alerts. Owns the eight channels — `yolo-private-cloud-backup`, `yolo-wd-cloud-backup`, `yolo-private-cloud-start-scratch`, `yolo-private-cloud-restore-scratch`, `yolo-private-cloud-resource-crunch`, `yolo-public-uptime` (the host-side yolo serving probe, `yolo-uptime-probe.sh`), plus two published by things that are **not bash** and so never call `ntfy_push`: `yolo-private-cloud-platform` (Alertmanager's webhook, kube-prometheus `manifests/alertmanager-secret.yaml`) and `yolo-grafana` (Grafana's contact point, the yolo repo's `grafana-alerting-yolo` ConfigMap) — plus `ntfy_topic_valid` (rejects unregistered topics), `ntfy_header_safe` (HTTP headers are latin1: an em dash in a Title fails *before* the request is sent) and `ntfy_push` (**always returns 0**, never writes stdout — a notification must never abort a backup or a DR run). `NTFY_DRY_RUN=1` prints instead of sending. Topics are **not secrets** (ntfy.sh topics are world-readable/writable) — so no message body may carry a secret or PII. The private `NTFY_URL` in `.env` is a *separate*, pre-registry topic used by `cluster-autostart.sh`/`vault-auto-unseal.sh`; leave it alone. See `docs/architecture.md` §7a. |
...
| `yolo-uptime-probe.sh` | **`*/5` cloud cron** → `yolo-public-uptime`, plus a **Sunday 06:07 `--selftest`** line. Probes YOLO's follower-facing serving path from the host: the public UI answers 2xx, and `/v1/market/session?exchange=XNYS` answers 2xx **with `"state"` in the body** — one request that exercises NPM → NodePort → api-gateway → robinstocks → quant-mongo, so a live proxy fronting a dead backend still reads DOWN. Exists because of the yolo **2026-08-03→08-07 post-mortem**: yolo's app alerting (promtail→Loki→Grafana rules) died WITH the cluster and paged nobody for four days; `alerting-pipeline-watch.sh` couldn't see it either (it watches kube-prometheus, not yolo's separate Loki stack or the serving path). **Reuses** the watch script's tested sustained/cooldown/recovery machine (sourced via its `AP_LIB_ONLY` seam — one state machine, one place for its bugs) and `ntfy-lib.sh`. The weekly `--selftest` proves the gate can still FAIL (missing path reads down, real API reads up, wrong body reads down, machine transitions) and pushes **only when the selftest itself fails**. Vantage, honestly: host-side covers cluster/NPM death, not the box dying — the same script installs unchanged on the dev box for that, and a hosted uptime service is the fully-external layer if ever wanted. State in `logs/.yolo-uptime-state`; `--status` / `--dry-run` as usual. Tests: `tests/test-yolo-uptime-probe.sh`. |
...
- **The admin vhosts are locked to a source allowlist, and you widen it by joining the
  tailnet — never by adding an IP.** `nginx/data/nginx/custom/http_top.conf` (SEC-EDGE-ALLOWLIST,
  2026-08-15) restricts jenkins/grafana/vault/registry/prometheus/ai/kube-api/NPM-admin to a
  trusted source set, because nginx routes on the **Host header, not DNS** — a vhost absent
  from public DNS is still reachable by anyone who sends its name, and the audit that
  prompted this found Vault disclosing its unsealed state and `/v2/_catalog` listing every
  tenant's images to the internet. To reach these from elsewhere, enrol the device in the
  tailnet (`tailscale-access.sh`); do **not** add its address. Three traps:
  (a) `213.48.246.115/32` in that geo block is **not** just "our public IP for convenience" —
  every in-cluster image pull hairpins in as that address, so deleting it is a cluster-wide
  `ImagePullBackOff`; (b) because subnet-route traffic is **SNATed**, a tailnet device also
  arrives as that address, so the per-device `100.x/32` entries grant nothing — **revoke in
  the Tailscale console, not by editing that file**; (c) test the deny branch only from a
  genuinely untrusted vantage (`docker run --network bridge …`), since loopback and the LAN
  are trusted and a `curl` from this host proves nothing. Full detail + the rejected
  alternatives (Cloudflare Access, split DNS, `tailscale serve`, exit node):
  nginx repo `docs/edge-exposure.md`.
